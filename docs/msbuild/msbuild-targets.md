...
description: Erfahren Sie, wie MSBuild mithilfe von Zielen Aufgaben gruppiert und so eine Aufteilung des Buildprozesses in kleinere Einheiten ermöglicht.
...
Ein Zielelement kann ein `Outputs`-Attribut enthalten, das Metadaten im Format %(\<Metadata>) angibt. Wenn dies der Fall ist, führt MSBuild das Ziel einmal für jeden eindeutigen Metadatenwert aus und gruppiert die Elemente, die diesen Metadatenwert aufweisen. Ein auf ein Objekt angewendeter
